...
export class GoogleStrategy extends PassportStrategy(Strategy, 'google') {
  ...
  constructor(
    ...
    if (!clientID || !clientSecret) {
      throw new InternalServerErrorException('Google OAuth Client ID or Secret not configured in .env file');
    }
  ...
  async validate(accessToken: string, refreshToken: string, profile: any, done: VerifyCallback): Promise<any> {
    ...
    try {
      ...
      if (!user) {
         throw new InternalServerErrorException('Could not find or create user after Google validation');
      }
      done(null, user); 
    } catch (error) {
