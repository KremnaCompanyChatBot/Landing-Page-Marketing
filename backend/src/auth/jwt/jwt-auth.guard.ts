...
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  handleRequest(err: any, user: any, info: any) {
    if (err || !user) {
      this.logger.error(
        `JWT Auth failed - Error: ${err?.message || 'none'}, User: ${!!user}, Info: ${info?.message || 'none'}`,
      );
      if (info?.message === 'No auth token') {
        throw new UnauthorizedException(
          'No authentication token provided. Please include Bearer token in Authorization header.',
        );
      }
